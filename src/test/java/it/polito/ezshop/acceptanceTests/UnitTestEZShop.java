...
public class UnitTestEZShop {
	...
	public void getterAndSetterOrderTestCase() {
		// Integer orderId, Integer balanceId, String productCode, Double pricePerUnit,
	...
	public void startDeleteSaleTransactionTestCase() {
		...
		List<TicketEntry> productList = new ArrayList<>();
		String[] date = (new Date()).toString().split(" ");
		SaleTransactionClass saleTransaction = new SaleTransactionClass(170, date[0], date[1], 0.0, "", 0.0,
				productList, "OPEN");
	...
	public void addAndDeleteCreditCard() {
		CreditCardClass cr1 = new CreditCardClass("9979871602207724", 100.0);
		Integer cardId = db.getLastId("creditCards");
		assertTrue(db.recordCreditCard(cardId+1, cr1));
		assertFalse(db.recordCreditCard(0, null));

		assertTrue(db.deleteCreditCard(cardId+1));
		assertFalse(db.deleteCreditCard(-1));
	}
